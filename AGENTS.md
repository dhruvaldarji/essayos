...
In Codex the entry skills in `agent-skills/` wrap these steps: `essayos`, `essay-init`,
`essay-ingest`, `essay-scan`, `essay-next`, `essay-status`, `essay-resume`, and `essay-lint`.
...
- The final prose must sound **human, not scientific**, and not like a model. Before you store any
  prose, run the `humanizer` skill (`agent-skills/humanizer/SKILL.md`) in embedded mode with the
  `VoiceModel` quotes as the writing sample. Science shapes the process only.

## Third-party skills

Two pinned skills live in `agent-skills/` and are part of the contract (CONVENTIONS §10):
